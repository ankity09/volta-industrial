...
### Build 2 — Databricks App (DEPLOYED + RUNNING, but has functional gaps — THE MAIN WORK)
...
- **Browser test (chrome devtools) findings — the punch list:**
  1. **404s on the core data endpoints** — client calls `/api/lines`, `/api/lines/summary`, `/api/lines/:id`, `/api/lines/:id/recommendation`, `/api/plants/map`, `/api/parts/search`, `/api/activity/recent`, POST `/api/lines/:id/work-order`, but **no server route implements them**. This is why the at-risk table is empty + KPIs show 0. **A fix is IN FLIGHT**: a databricks-apps-developer subagent is implementing `server/routes/lines.ts` (reusing server/db/queries/maintenance.ts helpers) + registering it. CHECK whether that landed (git log on build/volta-app; look for a "feat(server): implement /api/lines..." commit) and whether `npm run build:source` passes, then REDEPLOY.
  2. **3D canvas WebGL framebuffer errors** ("Attachment has zero size", 187 warnings) — the PlantFloor3D component renders into a zero-size container. Likely a CSS/layout issue (canvas parent has 0 height until data loads) or the bloom EffectComposer sized before layout. Fix after the data endpoints work (the canvas may just need the lines data + a sized container).
